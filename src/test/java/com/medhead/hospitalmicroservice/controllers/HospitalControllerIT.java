...
public class HospitalControllerIT {
    ...
    public void shouldGetAllHospitals() throws Exception {
        mockMvc.perform(get("/hospital/all")
                ...
                .andExpect(jsonPath("$[0].hospitalName", is("hospital1")))
                .andExpect(jsonPath("$[1].hospitalName", is("hospital2")));
    }
    ...
    public void shouldGetHospitalById() throws Exception {
        mockMvc.perform(get("/hospital/1")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.hospitalName", is("hospital1")))
                .andExpect(jsonPath("$.hospitalLatitude", is(51.5007)))
                .andExpect(jsonPath("$.hospitalLongitude", is(-0.1246)));
    }
    ...
    public void shouldFindClosestHospitalWithFreeBedsBySpeciality() throws Exception {
        ResultActions result = mockMvc.perform(get("/hospital/closest")
                ...
                .param("userLonStr", "-0.1246")
                .contentType(MediaType.APPLICATION_JSON));

        result.andExpect(status().isOk())
                .andExpect(jsonPath("$.size()", greaterThanOrEqualTo(1)))
                .andExpect(jsonPath("$[0].hospital.hospitalName", is("hospital1")));

        // Afficher le JSON de la réponse dans la console
